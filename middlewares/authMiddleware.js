...
export default function authMiddleware(req, res, next) {
	...
	if (!basicAuthorizationHeader) {
		log('Server Auth', `${req.ipAddress} tried to access ${req.path} [${req.method}] without authorization (req-id: ${req.id})`);
	...
	if (basicToken != process.env.API_TOKEN) {
		log('Server Auth', `${req.ipAddress} tried to access ${req.path} [${req.method}] with a invalid authorization (req-id: ${req.id})`);
